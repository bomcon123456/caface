...
class VisionTransformer(nn.Module):
    ...
    def make_spatial(self, x):
        ...
        spatial = torch.stack([mean.view(B, P), std.view(B, P)], dim=-1)
        return spatial

    def forward_features(self, x, return_styles=[]):
        styles = []
        spatials = []
        ...
        for idx, func in enumerate(self.blocks):
        ...
        if self.training and self.mask_ratio > 0:
            ...
            x = x_
        return torch.reshape(x, (B, self.num_patches * self.embed_dim)), styles + spatials

    def forward(self, x, return_intermediate=False, return_style=[]):
        x, styles = self.forward_features(x, return_style)
